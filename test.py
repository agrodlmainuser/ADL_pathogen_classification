...
class TestPathogen:
  ...
  def __init__(self, test_images_dir, trained_model_dir):
  ...
  def test_classification(self, IMAGE_SIZE):
    ...
    for leaf_folder in os.listdir(self.test_images_dir):
      for leaf_image in os.listdir(images_dir):
      if leaf_image.endswith("mp4"):
        continue
      else:
        leaf_image_path = f'{folder_dir}/{leaf_image}'
